...
In this article we will learn how to configure and use [`nvim-dap`][nvim-dap] which is
a *Debug Adapter Protocol client implementation for Neovim*. I will choose
`C/C++/Rust` languages as a demonstration but this will also apply to any
other language.
...
└── init.vim
````
And here is a note about the files inside `after/plugin/debugger`
...
```` lua
local M = {}local function bind(op, outer_opts)
...
dap.configurations.cpp = {
    {
        -- Change it "cppdbg" if you have vscode-cpptools
        type = "codelldb",
        request = "launch",
        -- You can change this to your executable,
        -- you can even define a function to prompt you to enter exec name.
        -- See args option
